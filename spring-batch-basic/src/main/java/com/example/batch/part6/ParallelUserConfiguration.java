...
import com.example.batch.part4.LevelUpJobExecutionListener;
import com.example.batch.part4.SaveUserTasklet;
import com.example.batch.part4.User;
import com.example.batch.part4.UserRepository;
...
public class ParallelUserConfiguration {
    ...
    public Flow splitFlow(@Value("#{jobParameters[date]}") String date) throws Exception {
        ...
        return new FlowBuilder<SimpleFlow>(JOB_NAME + "_splitFlow")
                ...
                .add(userLevelUpFlow, orderStatisticFlow(date))
                .build();
    }


    private Flow orderStatisticFlow(String date) throws Exception {
        return new FlowBuilder<SimpleFlow>(JOB_NAME + "_orderStatisticsFLow")
                .start(new JobParametersDecide("date"))
                .on(JobParametersDecide.CONTINUE.getName())
                .to(this.orderStatisticStep(date))
                .build();
    }

    @Bean(JOB_NAME + "_orderStatisticsStep")
    @JobScope
    public Step orderStatisticStep(@Value("#{jobParameters[date]}") String date) throws Exception {
